...
namespace _03 {
    class Program {
        ...
        private static Dictionary<string, int> RunWire (string path, Dictionary<string, int> previousWire = null, bool findMinimum = false, bool findSteps = false) {
            ...
            foreach (var op in path.Split (",")) {
                char direction = op[0];
                int distance = int.Parse (op.Substring (1));
                ...
                for (var i = 0; i < distance; i++) {
                    ...
                    if (previousWire != null && previousWire.ContainsKey(position)) {
                        var stepsToThisIntersection = previousWire[position] + totalSteps;

                        if (stepsToThisIntersection < minSteps) {
                            minSteps = stepsToThisIntersection;
                        }
