...
for noun in range (0,100):
    for verb in range (0,100):
        thisRun = [int(x) for x in puzzleInput.split(",")]
        thisRun[1] = noun
        thisRun[2] = verb

        calculate(thisRun)
        if (thisRun[0] == 19690720):
            print ('part2: ' + str(noun) + ', ' + str(verb))
            print ('part2 sum: ' + str((noun* 100) + verb))
            break
        else:
            continue
#break
